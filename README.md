...
history:
  ...
  record_errors: false        # true — писать строки с error (баланс null)
```
...
Очистка: самые старые записи удаляются **пачками по 1%** от числа строк в таблице; затем `incremental_vacuum`. Запускается при старте бота и после каждой записи.
...
В каждой строке истории: время опроса, имя сервиса, баланс, валюта, дата подписки, источник (например `grant` / `balance` у Cloud.ru).
