...
По умолчанию бот шлёт уведомление только при **появлении** нового алерта (низкий баланс, скорое окончание подписки, ошибка API). Чтобы после перезапуска контейнера не дублировать уже известные проблемы, включите `history` и секцию `alerts`:

```yaml
alerts:
  persist: true                 # хранить активные алерты в SQLite (нужен history.enabled)
  suppress_on_startup: true     # первый опрос после старта — без push
  error_confirm_failures: 2     # алерт error после N сбоев подряд (1 — сразу)
```
